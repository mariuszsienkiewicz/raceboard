<?php

declare(strict_types=1);

namespace App\Tests\Unit\RaceCatalog\Application\Handler;

use App\RaceCatalog\Application\Handler\RecalculateRaceAverageRatingHandler;
use App\RaceCatalog\Domain\Model\Race;
use App\RaceCatalog\Domain\Repository\RaceRepositoryInterface;
use App\RaceCatalog\Domain\Service\RaceRatingProviderInterface;
use App\Review\Domain\Event\ReviewAdded;
use App\Review\Domain\Event\ReviewRemoved;
use App\Shared\Domain\Model\RaceId;
use PHPUnit\Framework\MockObject\MockObject;
use PHPUnit\Framework\TestCase;

final class RecalculateRaceAverageRatingHandlerTest extends TestCase
{
    private RaceRatingProviderInterface&MockObject $raceRatingProvider;
    private RaceRepositoryInterface&MockObject $raceRepository;
    private RecalculateRaceAverageRatingHandler $handler;

    protected function setUp(): void
    {
        $this->raceRatingProvider = $this->createMock(RaceRatingProviderInterface::class);
        $this->raceRepository = $this->createMock(RaceRepositoryInterface::class);
        $this->handler = new RecalculateRaceAverageRatingHandler(
            $this->raceRatingProvider,
            $this->raceRepository,
        );
    }

    public function testRecalculatesAverageOnReviewAdded(): void
    {
        $race = Race::create(RaceId::generate(), 'Test Race', 'Test City', 'Test Voivodeship');

        $this->raceRatingProvider->expects($this->once())
            ->method('getAverageRating')
            ->with($race->getId())
            ->willReturn(4.5);
        $this->raceRepository->expects($this->once())
            ->method('findById')
            ->with($race->getId())
            ->willReturn($race);
        $this->raceRepository->expects($this->once())
            ->method('save')
            ->with($race);

        $this->handler->onReviewAdded(new ReviewAdded($race->getId()));

        $this->assertEqualsWithDelta(4.5, $race->getAverageRating(), 0.001);
    }

    public function testDoesNothingWhenRaceIsNotFound(): void
    {
        $raceId = RaceId::generate();

        $this->raceRatingProvider->expects($this->once())
            ->method('getAverageRating')
            ->with($raceId)
            ->willReturn(4.5);
        $this->raceRepository->expects($this->once())
            ->method('findById')
            ->with($raceId)
            ->willReturn(null);
        $this->raceRepository->expects($this->never())
            ->method('save');

        $this->handler->onReviewAdded(new ReviewAdded($raceId));
    }

    public function testClearsAverageWhenLastReviewIsRemoved(): void
    {
        $race = Race::create(RaceId::generate(), 'Test Race', 'Test City', 'Test Voivodeship');
        $race->updateAverageRating(4.5);

        $this->raceRatingProvider->expects($this->once())
            ->method('getAverageRating')
            ->with($race->getId())
            ->willReturn(null);
        $this->raceRepository->expects($this->once())
            ->method('findById')
            ->with($race->getId())
            ->willReturn($race);
        $this->raceRepository->expects($this->once())
            ->method('save')
            ->with($race);

        $this->handler->onReviewRemoved(new ReviewRemoved($race->getId()));

        $this->assertNull($race->getAverageRating());
    }

    public function testRecalculatesAverageWhenReviewIsRemovedButOthersRemain(): void
    {
        $race = Race::create(RaceId::generate(), 'Test Race', 'Test City', 'Test Voivodeship');
        $race->updateAverageRating(5.0);

        $this->raceRatingProvider->expects($this->once())
            ->method('getAverageRating')
            ->with($race->getId())
            ->willReturn(4.0);
        $this->raceRepository->expects($this->once())
            ->method('findById')
            ->with($race->getId())
            ->willReturn($race);
        $this->raceRepository->expects($this->once())
            ->method('save')
            ->with($race);

        $this->handler->onReviewRemoved(new ReviewRemoved($race->getId()));

        $this->assertEqualsWithDelta(4.0, $race->getAverageRating(), 0.001);
    }
}
